# src/tests/test_falhas_por_id_equipamento.py
...
from views.falhas_por_id_equipamento import (
    execute_sql_clickhouse,
    create_tables,
    insert_falhas_por_id_equipamento,
    create_view,
    falhas_por_id_equipamento_etl
)
...
def test_execute_sql_clickhouse():
    ...
    with patch('views.falhas_por_id_equipamento.Client') as mock_client_class:
...
def test_create_tables():
    ...
    with patch('views.falhas_por_id_equipamento.execute_sql_clickhouse') as mock_execute_sql:
        # Act: Chama a tarefa
        ...
        assert result == "Tabela intermediate_falhas_por_id_equipamento criada (ou já existente)."

def test_insert_falhas_por_id_equipamento():
    """
    Este teste verifica se a tarefa insert_falhas_por_id_equipamento chama a função execute_sql_clickhouse e retorna a mensagem correta.
    """
    # Arrange: Mock da função execute_sql_clickhouse
    with patch('views.falhas_por_id_equipamento.execute_sql_clickhouse') as mock_execute_sql:
        # Act: Chama a tarefa
        result = insert_falhas_por_id_equipamento()

        # Assert: Verifica se a função execute_sql_clickhouse foi chamada
        assert mock_execute_sql.called
        # Verifica se o retorno é o esperado
        assert result == "Dados de falhas por ID_EQUIPAMENTO inseridos na tabela intermediate_falhas_por_id_equipamento."
...
def test_create_view():
    ...
    with patch('views.falhas_por_id_equipamento.execute_sql_clickhouse') as mock_execute_sql:
        ...
        assert mock_execute_sql.called
        # Verifica se o retorno é o esperado
        assert result == "View view_falhas_por_id_equipamento criada (ou já existente)."

def test_falhas_por_id_equipamento_etl():
    """
    Este teste verifica se o flow falhas_por_id_equipamento_etl chama as tarefas na ordem correta.
    """
    # Arrange: Mock das tarefas
    with patch('views.falhas_por_id_equipamento.create_tables') as mock_create_tables, \
         patch('views.falhas_por_id_equipamento.insert_falhas_por_id_equipamento') as mock_insert_falhas, \
         patch('views.falhas_por_id_equipamento.create_view') as mock_create_view:
